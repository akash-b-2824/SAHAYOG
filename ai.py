...
def index():
    global job_lat, job_lon,job_skills,job_gender,job_age,top_labor
    if request.method == 'POST':
        ...
        y=main(float(job_lat), float(job_lon),job_skills,job_gender,int(job_age))
        print(y)
        top_labor = {
                'name': y.iloc[0]['Name'],
                'phone': y.iloc[0]['Phone'],
                'probability': round(y.iloc[0]['Hired_Probability'] * 100, 2)
            }
    ...
    return render_template('form.html', top_labor=top_labor)
app.run(debug=True)