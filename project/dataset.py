...
phone_numbers = ["9538169612", "9591442990", "7411151257"]  # your fixed phone numbers

for _ in range(1000):
    name = fake.name()
    adhar = str(random.randint(100000000000, 999999999999))  # 12-digit
    phone = random.choice(phone_numbers)  # choose phone from the list
    skills = ";".join(random.sample(skill_pool, random.randint(1, 4)))
    age = random.randint(18, 60)
    gender = random.choice(genders)
    language = random.choice(languages)
    latitude = round(random.uniform(20.0, 23.0), 6)
